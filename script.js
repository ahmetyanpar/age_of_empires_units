...
window.onload = function listUnits() {
    $('#units').html('');
    fillUnits(units);
};

function fillUnits(arr) {
    $.each(arr,function(_index, value) {
        var id = value.id;
        var name = value.name;
        var age = value.age;
        var costs = value.cost;
        if (costs === null) {
            var costs = "0";
            var dataCosts = null;
        } else if (costs["Wood"] == undefined) {
            var costGold = `Gold: ${value.cost["Gold"]}`;
            var costFood = `Food: ${value.cost["Food"]}`;
            var costs = `${costGold}, ${costFood}`;
            dataCosts = `data-food=${value.cost["Food"]} data-gold=${value.cost["Gold"]}`;
        } else if (costs["Food"] == undefined) {
            var costWood = "Wood: " + value.cost["Wood"];
            var costGold = `Gold: ${value.cost["Gold"]}`;
            var costs = `${costWood}, ${costGold}`;
            dataCosts = `data-wood=${value.cost["Wood"]} data-gold=${value.cost["Gold"]}`;
        } else if (costs["Gold"] == undefined) {
            var costWood = `Wood: ${value.cost["Wood"]}`;
            var costFood = `Food: ${value.cost["Food"]}`;    
            var costs = `${costWood}, ${costFood}`;
            dataCosts = `data-wood=${value.cost["Wood"]} data-Food=${value.cost["Food"]}`;
        };
        var unit = `
                  <tr class="unit" ${dataCosts}>
                    <th class="id" scope="row" onclick="unitDetail()">${id}</th>
                    <td class="name">${name}</td>
                    <td class="age">${age}</td>
                    <td class="costs">${costs}</td>
                  </tr>
        `
        $('#units').append(unit);
    });
...
function unitDetail() {
    ...
    console.log(units[id-1]);
    var unitId = units[id-1].id;
    var name = units[id-1].name;
    var description = units[id-1].description;
    var expansion = units[id-1].expansion;
    var age = units[id-1].age;
    var costWood = units[id-1].cost["Wood"];
    var costFood = units[id-1].cost["Food"];
    var costGold = units[id-1].cost["Gold"];
    var buildTime = units[id-1].build_time;
    var reloadTime = units[id-1].reload_time;
    var attackDelay = units[id-1].attack_delay;
    var movementRate = units[id-1].movement_rate;
    var lineOfSight = units[id-1].line_of_sight;
    var hitPoints = units[id-1].hit_points;
    var range = units[id-1].range;
    var attack = units[id-1].attack;
    var armor = units[id-1].armor;
    var accuracy = units[id-1].accuracy;
    var unitDetail = `
    <h2 class="text-center">Unit Detail Page</h2>
    <table class="table table-dark table-striped mx-auto w-50 mt-5">
      ...
      `
    $('.main').empty();
    $('.main').append(unitDetail);
}




function ageAllFilter() {
    $('#units').empty();
    fillUnits(units);    
};

function ageDarkFilter() {
    $('#units').empty();
    var ageDarkUnits = units.filter( element => element.age == "Dark");
    fillUnits(ageDarkUnits);
};

function ageFeudalFilter() {
    $('#units').empty();
    var ageFeudalUnits = units.filter( element => element.age == "Feudal");
    fillUnits(ageFeudalUnits);
};

function ageCastleFilter() {
    $('#units').empty();
    var ageCastleUnits = units.filter( element => element.age == "Castle");
    fillUnits(ageCastleUnits);
};

function ageImperialFilter() {
    $('#units').empty();
    var ageImperialUnits = units.filter( element => element.age == "Imperial");
    fillUnits(ageImperialUnits);
};
...
$(function() {
    $('#slider-wood').slider({
        ...
        values: [0, 200],
        slide: (_event, ui) => {
            $("#amount-wood").val(ui.values[0] + " - " + ui.values[1]);
            var mi = ui.values[0];
            var mx = ui.values[1];
            filterWood(mi, mx);
        }
    ...
    $('#slider-gold').slider({
        ...
        values: [0, 200],
        slide: (_event, ui) => {
            $("#amount-gold").val(ui.values[0] + " - " + ui.values[1]);
            var mi = ui.values[0];
            var mx = ui.values[1];
            filterGold(mi, mx);
        }
    ...
    $('#slider-food').slider({
        ...
        values: [0, 200],
        slide: (_event, ui) => {
            $("#amount-food").val(ui.values[0] + " - " + ui.values[1]);
            var mi = ui.values[0];
            var mx = ui.values[1];
            filterFood(mi, mx);
        }
...
function filterWood(minWood, maxWood) {
$("#units .unit").hide().filter(function() {
    var wood = parseInt($(this).data("wood"), 10);
    return wood >= minWood && wood <= maxWood;
}).show();
};

function filterFood(minWood, maxWood) {
$("#units .unit").hide().filter(function() {
    var food = parseInt($(this).data("food"), 10);
    return food >= minWood && food <= maxWood;
}).show();
};

function filterGold(minGold, maxGold) {
$("#units .unit").hide().filter(function() {
    var gold = parseInt($(this).data("gold"), 10);
    return gold >= minGold && gold <= maxGold;
}).show();
};

jQuery(function () {
    jQuery("#units .unit .name").click(function () {
        console.log(units);
    });
});
